...
if (user) {
	...
	if (user.phoneNumber === submittedPhoneNumber) {
		...
		function sendPasswordResetLink(email, url) {
			...
			params._from = "openidm@authconnect.com";
			params._to = email;
		...
		email=  user.email;
		url = "http://openidm.authconnect.com:8080/openidm/endpoint/passwordReset?userName=" + userName + "&phoneNumber=" + phoneNumber + "&password=";
